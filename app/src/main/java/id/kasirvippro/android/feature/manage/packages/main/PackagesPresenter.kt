...
class PackagesPresenter(val context: Context, val view: PackagesContract.View) : BasePresenter<PackagesContract.View>(),
    PackagesContract.Presenter, PackagesContract.InteractorOutput {

    private var interactor = PackagesInteractor(this)
...
//        }
    ...
    override fun updateStore(data: Store?) {
        store = data
        view.setStoreName(data)
    }

    override fun checkTunai(name:String,price:String) {
        if(price.isBlank() || price.isEmpty()){
            view.showMessage(999,"Price cannot be empty")
            return
        }

        if(name.isBlank() || name.isEmpty()){
            view.showMessage(999,"Name Packages are required")
            return
        }
        val req = RequestPackages()
        req.price = price.toDouble()
        req.name = name
    ...
    private fun getBarang():List<RequestPackages.Barang>{
        val list = ArrayList<RequestPackages.Barang>()
        if(carts.size == 0){
            return list
        }
        for(cart in carts.values){
            val barang = RequestPackages.Barang()
