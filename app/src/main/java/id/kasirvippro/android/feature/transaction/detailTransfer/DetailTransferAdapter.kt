...
class DetailTransferAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val listProduct = mutableListOf<DetailTransfer.Data>()
    ...
    fun setItems(listProduct: List<DetailTransfer.Data>?) {
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bindData(data: DetailTransfer.Data, position: Int) {
            val decimal = AppConstant.DECIMAL.getDecimalData()

            nameTv.htmlText("${data.name_product}")
            countTv.text = "${Helper.convertToCurrency(data.amount!!)}x"
            priceTv.text = "@${Helper.convertToCurrency(data.price!!)}"
            if(decimal=="No Decimal") {
                subtotalTv.text = AppConstant.CURRENCY.getCurrencyData() + Helper.convertToCurrency(data.totalprice!!)
            }else{
