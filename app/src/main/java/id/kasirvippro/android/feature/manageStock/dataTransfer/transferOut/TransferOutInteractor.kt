...
class TransferOutInteractor(var output: TransferOutContract.InteractorOutput?) : TransferOutContract.Interactor {
    ...
    override fun callGetHistoryAPI(context: Context, restModel: TransactionRestModel, awal:String, akhir:String, status:String) {
        ...
        disposable.add(restModel.getTransferOut(key!!,awal,akhir,status).subscribeWith(object : DisposableObserver<List<HistoryTransfer>>() {
    ...
    override fun callGetSearchAPI(context: Context, restModel: TransactionRestModel, id: String) {
        val key = appSession.getToken(context)
        disposable.add(restModel.searchTransactionOut(key!!,id).subscribeWith(object : DisposableObserver<List<Transfer>>() {
