...
class StoreInteractor(var output: StoreContract.InteractorOutput?) :
    StoreContract.Interactor {
    ...
    override fun callUpdateAPI(context: Context,model:StoreRestModel,id:String,name:String,email:String,phone:String,address:String,pajak:String,service:String,currency:String,footer:String,shift:String,img:String?) {
        val key = appSession.getToken(context)
        disposable.add(model.updateStore(key!!,id,name,email,phone,address,pajak,service,currency,footer,shift,img).subscribeWith(object : DisposableObserver<Message>() {
