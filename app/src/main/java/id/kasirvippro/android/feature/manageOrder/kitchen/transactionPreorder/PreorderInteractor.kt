...
class PreorderInteractor(var output: PreorderContract.InteractorOutput?) : PreorderContract.Interactor {
    ...
    override fun callGetHistoryAPI(context: Context, restModel: TransactionRestModel,awal:String,akhir:String) {
        val key = appSession.getToken(context)
        disposable.add(restModel.getTransactionPreOrder(key!!,awal,akhir).subscribeWith(object : DisposableObserver<List<HistoryTransaction>>() {
    ...
    override fun callGetSearchAPI(context: Context, restModel: TransactionRestModel, id: String) {
        val key = appSession.getToken(context)
        disposable.add(restModel.searchPreorder(key!!,id).subscribeWith(object : DisposableObserver<List<Transaction>>() {
