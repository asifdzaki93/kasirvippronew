...
class TransferPresenter(val context: Context, val view: TransferContract.View) : BasePresenter<TransferContract.View>(),
    TransferContract.Presenter, TransferContract.InteractorOutput {

    private var interactor = TransferInteractor(this)
...
//        }
    ...
    override fun checkTunai() {
        if(store == null){
            view.showMessage(999,"Data Store has not been filled")
            return
        }

        val total = view.getTotalValue()
        val req = RequestTransfer()
        req.payment_type = 1
        req.total_order = total.toInt()
        req.id_store = store?.id_store
    ...
    private fun getBarang():List<RequestTransfer.Barang>{
        val list = ArrayList<RequestTransfer.Barang>()
        if(carts.size == 0){
            return list
        }
        for(cart in carts.values){
            val barang = RequestTransfer.Barang()
