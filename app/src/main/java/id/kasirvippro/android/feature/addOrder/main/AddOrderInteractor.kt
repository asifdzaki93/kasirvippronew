...
class AddOrderInteractor(var output: AddOrderContract.InteractorOutput?) :
    AddOrderContract.Interactor {
    ...
    override fun callOrderAPI(context: Context, restModel: TransactionRestModel, req: RequestAddOrder) {
        val key = appSession.getToken(context)
        req.key = key
        disposable.add(restModel.addorder(req).subscribeWith(object : DisposableObserver<Order>() {
