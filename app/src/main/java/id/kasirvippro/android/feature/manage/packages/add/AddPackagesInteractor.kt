...
class AddPackagesInteractor(var output: AddPackagesContract.InteractorOutput?) :
    AddPackagesContract.Interactor {
    ...
    override fun callOrderAPI(context: Context, restModel: TransactionRestModel, req: RequestPackages) {
        val key = appSession.getToken(context)
        req.key = key
        disposable.add(restModel.addpackages(req).subscribeWith(object : DisposableObserver<Order>() {
