...
class SellAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
       ...
       // private val addOnTv = view.tv_addon
        ...
        fun bindData(data: Cart, position: Int) {
           ...
           // priceTv.text = AppConstant.CURRENCY.getCurrencyData() + "${Helper.convertToCurrency(product?.selling_price!!)}"
            ...
            val alertstock = product.alertstock!!.toDouble()
            if(decimal=="No Decimal") {
            ...
            if(product.packages == "YES") {
                noteTv.text = "${product.description}"
            }else{
                noteTv.text = "${data.note}"
            }


            if(product.have_stock == "0"){
                ...
                if(stock < alertstock){
                    callback?.onStock(data,position)
                }
            }
    ...
    interface ItemClickCallback{
        ...
        fun onStock(data: Cart, position: Int)
    }
}