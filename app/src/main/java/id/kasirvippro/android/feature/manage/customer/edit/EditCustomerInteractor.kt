...
class EditCustomerInteractor(var output: EditCustomerContract.InteractorOutput?) : EditCustomerContract.Interactor {
    ...
    override fun callEditCustomerAPI(context: Context, model: CustomerRestModel, id:String, name: String,email:String,phone:String,address:String,customercode:String) {
        val key = appSession.getToken(context)
        disposable.add(model.update(key!!,id,name,email,phone,address,customercode).subscribeWith(object : DisposableObserver<Message>() {
