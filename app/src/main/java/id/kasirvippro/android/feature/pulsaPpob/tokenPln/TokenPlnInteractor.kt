...
class TokenPlnInteractor(var output: TokenPlnContract.InteractorOutput?) : TokenPlnContract.Interactor {
    ...
    override fun callPrefixAPI(context: Context, restModel: PulsaPpobRestModel, jenis: String, search: String) {
        ...
        disposable.add(restModel.searchToken(key!!,jenis,search).subscribeWith(object : DisposableObserver<List<PulsaPpob>>() {
