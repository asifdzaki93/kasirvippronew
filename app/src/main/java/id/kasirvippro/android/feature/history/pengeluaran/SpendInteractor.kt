...
class SpendInteractor(var output: SpendContract.InteractorOutput?) : SpendContract.Interactor {
    ...
    override fun callGetHistoryAPI(context: Context, restModel: TransactionRestModel,awal:String,akhir:String) {
        ...
        disposable.add(restModel.historySpend(key!!,awal,akhir).subscribeWith(object : DisposableObserver<List<HistoryTransaction>>() {
    ...
    override fun callGetSearchAPI(context: Context, restModel: TransactionRestModel, id: String) {
        val key = appSession.getToken(context)
        disposable.add(restModel.searchSpend(key!!,id).subscribeWith(object : DisposableObserver<List<Transaction>>() {
