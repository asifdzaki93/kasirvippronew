...
class TransferInteractor(var output: TransferContract.InteractorOutput?) :
    TransferContract.Interactor {
    ...
    override fun callOrderAPI(context: Context, restModel: TransactionRestModel, req: RequestTransfer) {
        val key = appSession.getToken(context)
        req.key = key
        disposable.add(restModel.transfer(req).subscribeWith(object : DisposableObserver<Order>() {
