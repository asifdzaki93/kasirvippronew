...
class DataHistoryAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
        ...
        private val unitIv = view.tv_unit

        @SuppressLint("SetTextI18n")
        fun bindData(data: DetailHistory, position: Int) {
            val decimal = AppConstant.DECIMAL.getDecimalData()
            val date = Helper.getDateFormat(itemView.context,data.date!!,"yyyy-MM-dd","EEE, dd MMMM yyyy")
            dateIv.text = date
            nameTv.text = "${data.name_product}"
            detailTv.text = "${data.detail}"
            unitIv.text = "${data.unit}"
                ...
                if(status == "0"){
                    ...
                    unitIv.setTextColor(ContextCompat.getColor(itemView.context,R.color.bg_header_success))
                }
                else{
                    if(decimal=="No Decimal") {
                        stockTv.text = "-${data.stock!!}"
                    ...
                    unitIv.setTextColor(ContextCompat.getColor(itemView.context,R.color.vermillion))
                }
