...
class ListRawMaterialAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private val listProduct = mutableListOf<ReportRawMaterial>()
    ...
    fun setItems(listProduct: List<ReportRawMaterial>?) {
    ...
    inner class ViewHolder(view: View) : RecyclerView.ViewHolder(view) {
       ...
       // private val statusTv = view.tv_status
        ...
        fun bindData(data: ReportRawMaterial, position: Int, type:Int) {

            if(HEADER == type){
                headerDateTv.text = Helper.getDateFormat(itemView.context,data.date!!,"yyyy-MM-dd","EEE, dd MMMM yyyy")
                headerTotalTv.text = Helper.convertToCurrency(data.totalsales!!)
                ...
                val decimal = AppConstant.DECIMAL.getDecimalData()
                    totalTv.text = Helper.convertToCurrency(data.totalsales!!)
                if(decimal=="No Decimal") {
                    val price = Helper.convertToCurrency(data.payment!!)
                    methodTv.text = data.detail
                }else{
    ...
    interface ItemClickCallback{
        fun onClick(data: ReportRawMaterial)
    }
}