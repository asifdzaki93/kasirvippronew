...
public class PrinterUtil {
    ...
    public static void print(BluetoothSocket bluetoothSocket, DetailTransaction data, Store store, boolean isPremium, Bitmap drawable) {
        try {
            PrinterUtil pUtil = new PrinterUtil(bluetoothSocket.getOutputStream(), "GBK");
            printHeader(pUtil,data,store);
            printItem(pUtil,data);
            printInfo(pUtil,data);
            printQrCode(pUtil,data,drawable);
            printNote(pUtil,data);
    ...
    static void printHeader(PrinterUtil pUtil, DetailTransaction data, Store store) {
        try {
            String newLine = "\n";
            String no = "No Invoice:";
            String tanggal = "Tanggal:";
            String method = "Pembayaran:";
            String customer = "Pelanggan:";
            String supplier = "Suplier:";
...
//                operatorValue = "-";
//                pUtil.printLine();
//                String text = operator+getWhiteSpace(struk.getPaper() - operator.length() - operatorValue.length()) + operatorValue;
//                pUtil.writeWithFormat(text.getBytes(),new Formatter().small().get(),Formatter.leftAlign());
//            }
...
//            }
    ...
    private static void printInfo(PrinterUtil pUtil, DetailTransaction data) {
        try {
            ...
            if(struk.getTax() != null){
                double taxDouble = Double.parseDouble(struk.getTax());
                if(taxDouble > 0){
                    String taxValue = convertToCurrency(struk.getTax());
            ...
            String totalValue = convertToCurrency(struk.getTotalorder());
            ...
            if(struk.getTotalpay() != null){
                ...
                if(bayarDouble > 0){
                    String bayarValue = convertToCurrency(struk.getTotalpay());
            ...
            if(struk.getChangepay() != null){
                ...
                if(kembalianDouble > 0){
                    String kembalianValue = convertToCurrency(struk.getChangepay());
    ...
    private static void printQrCode(PrinterUtil pUtil, DetailTransaction data, Bitmap drawable) {
        try {
            DetailTransaction.Struk struk = data.getStruk();
            assert struk != null;
            String qrcode = struk.getLink_order();
    ...
    private static void printNote(PrinterUtil pUtil, DetailTransaction data) {
        try {
            pUtil.printLine();
            pUtil.printAlignment(0);
            pUtil.printText("Customer Note");
            pUtil.printLine();

            DetailTransaction.Struk struk = data.getStruk();
            assert struk != null;
            String note = struk.getNote();

            pUtil.printLine(1);
            pUtil.writeWithFormat(note.getBytes(),new Formatter().small().get(),Formatter.leftAlign());
            pUtil.printLine();
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void printFooter(PrinterUtil pUtil, DetailTransaction data) {
        try {
            DetailTransaction.Struk struk = data.getStruk();
            assert struk != null;
    ...
    private static void printThanks(PrinterUtil pUtil) {
        try {
    ...
    private static void printPowered(PrinterUtil pUtil) {
        try {
            String info = "Powered By Kasir Pro";
    ...
    private static void printItem(PrinterUtil pUtil, DetailTransaction data) throws IOException {
        ...
        for(DetailTransaction.Data model : items){
            try {
                ...
                String name = convertToCurrency(model.getAmount())+"x"+"@"+convertToCurrency(model.getPrice());
    ...
    private static byte[] getLogo(Bitmap drawable){
        Bitmap resize = Bitmap.createScaledBitmap(drawable, 250, 250, false);
